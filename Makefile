...
bench-monitor:
	docker compose --compatibility -f docker-compose.yml -f references/docker/docker-compose.monitor.yml down
	docker compose --compatibility -f docker-compose.yml -f references/docker/docker-compose.monitor.yml up --build --force-recreate -d
	@i=0; until curl -sf http://localhost:$(PORT)/ready >/dev/null 2>&1; do \
		printf '.'; sleep 1; i=$$((i+1)); \
		[ $$i -ge $(READY_TIMEOUT) ] && echo " timeout" && exit 1; \
	done; echo " ready"
