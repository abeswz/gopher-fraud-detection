...
var ErrParse = errors.New("search: corpus parse error")
...
// ParseCorpusRefs parses a flat JSON array of {vector:[14f], label:"fraud|legit"}.
// maxRefs is a capacity hint; the function stops if the array ends first.
func ParseCorpusRefs(buf []byte, maxRefs int) ([]Ref, error) {
	refs := make([]Ref, 0, maxRefs)
	p, n := 0, len(buf)
	skipWs := func() {
		for p < n && isWs(buf[p]) {
			p++
